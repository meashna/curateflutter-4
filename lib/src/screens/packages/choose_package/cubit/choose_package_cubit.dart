...
class ChoosePackageCubit extends Cubit<ChoosePackageState> {
  ...
  final Set<String> _kIds = <String>{
    /*'com.curate.planTest', 'com.curate.subTest1', 'com.curate.testNonConsumable', 'com.curate.TestConsumable', 'com.curate.plan3Month', 'com.curate.plan6Month', 'com.curate.plan12Month'*/
  };
  ...
  ChoosePackageCubit(this.argumentData, this.context)
      : super(ChoosePackageLoading()) {
  ...
  void setSelectedIOSproduct(Data? element) {
    ...
    purchaseIOSProduct(products.firstWhere(
        (inAppElement) => (element?.iosPlanId ?? "") == inAppElement.id));
  }
      ...
      purchaseDetailsList.forEach((PurchaseDetails purchaseDetails) async {
        if (purchaseDetails.status == PurchaseStatus.pending) {
          ...
          if (purchaseDetails.status == PurchaseStatus.error) {
            ...
            print(purchaseDetails.status);
            print(purchaseDetails.error);
            AppUtils.showToast(purchaseDetails.error == null
                ? "In app error"
                : ('Error Code ${purchaseDetails.error!.code} Error Message ${purchaseDetails.error!.message}'));
          ...
          if (purchaseDetails.pendingCompletePurchase) {
            ...
            const env =
                AppConstants.baseUrlDev == "https://curateapi.illuminz.com/"
                    ? "SANDBOX"
                    : "PRODUCTION";
            Map<String, dynamic> queryData = {
              "transactionId": purchaseDetails.purchaseID,
              "productId": selectedElement?.id,
              "env": env
            };
            ...
            if (response.status == Status.COMPLETED) {
              if (response.data?.flag ==
                  ApiConstants.paymentInAppPurchaseSuccess) {
                RouteNavigator.popAllAndPushNamedReplacement(
                    context, AppScreens.mainScreen);
              } else {
                AppUtils.showToast(
                    "get Apple transaction api flag: ${response.data?.flag}");
              }
            } else {
              AppUtils.showToast(
                  "get Apple transaction api error: ${response.message}");
            }
          }
        }
      });
    ...
    if (checkNetwork) {
      try {
        ...
        if (response.status == Status.COMPLETED) {
          ...
          return {
            "status": ApiConstants.paymentStatusFailed,
